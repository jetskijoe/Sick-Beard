...
class TVShow(object):
    ...
    def loadNFO (self):
        ...
        if showXML.findtext('studio') is not None:
            self.network = showXML.findtext('studio')
        if self.network is None and showXML.findtext('network') is not None:
            self.network = ""
        if showXML.findtext('genre') is not None:
